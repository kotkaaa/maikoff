...
class OAuth {
    ...
    public function getFBtoken($soc){
        return '<script type="text/javascript">
                ...
                var token = href.mb_substr(href.indexOf("access_token")+13);
                token = token.mb_substr(0, token.indexOf("&"));
    ...
    public function getSocUser($token, $soc='', $user_id=0){
        ...
        if($soc=="fb") {
            ...
            if(!empty($data)) {
                ...
                $arUser['bdate']        = (!empty($data->birthday))? mb_substr($data->birthday, 6).'-'.mb_substr($data->birthday, 0, 2).'-'.mb_substr($data->birthday, 3, 2): '';
