...
function RemoveAccents($string) {
	// Revised version by markstewardШhotmail*com
	return strtr(strtr($string, 'ЉЋљћџАБВГДЕЗИЙКЛМНОПСТУФХЦШЩЪЫЬЭабвгдезийклмнопстуфхцшщъыьэя', 'SZszYAAAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyy'), array('Ю' => 'TH', 'ю' => 'th', 'Р' => 'DH', 'р' => 'dh', 'Я' => 'ss', 'Њ' => 'OE', 'њ' => 'oe', 'Ж' => 'AE', 'ж' => 'ae', 'µ' => 'u'));
}
...
if (!empty($_REQUEST['scan']) || !empty($_REQUEST['newscan']) || !empty($_REQUEST['rescanerrors'])) {
	...
	while ($row = mysql_fetch_array($result)) {
		...
		$PatternFilename = str_replace('/',  'Ч',  $PatternFilename);

		$PatternFilename = str_replace('?',  'ї',  $PatternFilename);
...
function BetterUCwords($string) {
	...
	for ($i = 1; $i < $stringlength; $i++) {
		...
		} elseif (preg_match('#^[\'A-Za-z0-9А-я]$#', $string{$i - 1})) {
