...
class Validator {
    ...
    public function validateArray($subject, $description = '', $key = '') {
        $valid = (is_array($subject) and !empty($subject));
        if (!$valid) $this->addError($description, $key);
        return $valid;
    }
    ...
    public function validatePhone($subject, $description = '', $key = '', $pattern = false, $minln=7, $maxln=17) {
        if($pattern === false)
            $pattern = "/^[0-9]{{$minln},{$maxln}}$/u";
        return $this->validateByPattern($pattern, $subject, $description, $key);
    }

    public function validateLogin($subject, $description = '', $key = '', $pattern = false, $minln=3, $maxln=40) {
        if($pattern === false)
            $pattern = "/^[a-z0-9_]{{$minln},{$maxln}}$/u";
        return $this->validateByPattern($pattern, $subject, $description, $key);
    }

    public function validatePassword($subject, $description = '', $key = '', $pattern = false, $minln=6, $maxln=32) {
        if($pattern === false)
            $pattern = "/^[A-Za-z0-9]{{$minln},{$maxln}}$/u";
        return $this->validateByPattern($pattern, $subject, $description, $key);
    }
