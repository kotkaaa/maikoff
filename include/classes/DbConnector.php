...
class DbConnector extends SystemComponent {
    ...
    public function fetchObject() {
        return is_resource($this->result) ? mysql_fetch_object($this->result) : false;
    }

    public function screenData($str, $bApplyTrim=false){
        if ($bApplyTrim) $str = trim($str);
        if (empty($str) or is_bool($str)) return $str;
        if ($str) $str = stripslashes($str);
        if ($str) $str = htmlspecialchars($str, ENT_QUOTES, WLCMS_SYSTEM_ENCODING);
        if ($str) $str = addslashes($str);
        return $str;
    }
    ...
    public function unScreenData($str, $bApplyTrim=false){
        ...
        if (empty($str) or is_bool($str)) return $str;
        if ($str) $str = stripslashes($str);
        if ($str) $str = htmlspecialchars_decode($str, ENT_QUOTES);
        return $str;
    }
