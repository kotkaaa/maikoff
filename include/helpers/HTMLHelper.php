...
class HTMLHelper {
     ...
     * @example array('яблоко', 'яблока', 'яблок')
     * @return String
     */
    ...
    public function getContactPhones($phones) {
        return PHPHelper::getContactPhones(trim($phones));
    }

    public function shortenColorTitle($title = "", $max = 10, $pre = 5, $pad = 2) {
        return PHPHelper::shortenColorTitle($title, $max, $pre, $pad);
    }
    ...
    public function getLastWatched(UrlWL $UrlWL) {
    ...
    public function getFilterMenuLevel($UrlWL, $category) {
        ...
        if(mysql_num_rows($query) > 0) {
            while ($row = mysql_fetch_assoc($query)) {
                ...
                // color
                if($row['tid'] == UrlFilters::TYPE_COLOR) {
                    $row['type'] = 'brand';
                    $sel = 'SELECT b.*, COUNT(c.`id`) AS `cnt` FROM `'.COLORS_TABLE.'` b ';
                    $jn = 'LEFT JOIN `'.CATALOG_TABLE.'` c ON(b.`id` = c.`color_id`) ';
                    $whr = 'WHERE c.`cid`='.$category['id'].' AND c.`active`>0 ';
                    $ord = 'GROUP BY b.`id` ORDER BY b.`order`';
                    $qry = $sel.$jn.$whr.$ord;
                    $res = mysql_query($qry);
                    if(mysql_num_rows($res) > 0) {
                ...
                elseif ($row['tid'] == UrlFilters::TYPE_TEXT) {
                    $row['type'] = 'simple';
                    $sel = 'SELECT pa.*, COUNT(c.`id`) AS `cnt` FROM `'.MODEL_ATTRIBUTES_TABLE.'` pa ';
                    $jn = 'LEFT JOIN `'.ATTRIBUTES_TABLE.'` a ON(a.`id` = pa.`aid`) LEFT JOIN `'.CATALOG_TABLE.'` c ON(pa.`mid` = c.`model_id`) LEFT JOIN `'.CATEGORY_ATTRIBUTES_TABLE.'` ca ON(ca.`aid` = a.`id`) ';
                ...
                elseif ($row['tid'] == UrlFilters::TYPE_NUMBER) {
                    ...
                    $jn  = 'LEFT JOIN `'.FILTERS_TABLE.'` f ON(f.`id` = r.`fid`) LEFT JOIN `'.MODEL_ATTRIBUTES_TABLE.'` pa ON(pa.`aid` = f.`aid`) ';
                    $whr = 'WHERE f.`id`='.$row['id'].' AND pa.`mid` IN(SELECT DISTINCT `model_id` FROM `'.CATALOG_TABLE.'` WHERE `cid`='.$category['id'].') ';
