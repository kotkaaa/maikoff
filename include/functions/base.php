...
function getCacheId($def_cache_id=''){
    ...
    } return $def_cache_id.($def_cache_id!='' ? '-' : '').mb_substr(md5($cache_id.$lang), 0, 17);
}
...
function getTemplateFileName($ajax, $catid=0) {
    ...
    if (WLCMS_ZONE=='BACKEND') {
        ...
        return ($ajax ? 'ajax' : ($catid==$arrModules["checkout"]["id"] ? 'checkout' : 'index')).'.tpl';
    }
}
...
function screenData($item, $bApplyTrim=true) {
    ...
    } elseif (!is_bool($item) and $item) {
        if ($bApplyTrim) $item = trim($item);
        if ($item) $item = stripslashes($item);
        if ($item) $item = htmlspecialchars($item, ENT_QUOTES);
        if ($item) $item = addslashes($item);
    } return $item;
}
...
function unScreenData($item, $bApplyTrim=true) {
    ...
    if (is_array($item)) {
        ...
        if ($bApplyTrim) $item = trim($item);
        if ($item) $item = stripslashes($item);
        if ($item) $item = htmlspecialchars_decode($item, ENT_QUOTES);
    } return $item;
}
...
function createUniqueFileName($dir, $ext, $filename='', $fileprefix = '', $tolower = true){
    $file = ((empty($filename)) ? setFilePathFormat($fileprefix, $tolower).mb_substr(md5(uuid()),10) : setFilePathFormat($fileprefix.$filename, $tolower)).".".$ext;
    if(checkFileExist($dir, $file)) $file = createUniqueFileName($dir, $ext, '', $fileprefix, $tolower);
    return $file;
}

function createUniqueDirName($root, $dirname='', $maxLenth=63){
    if(empty($dirname)) $dirname = mb_substr(md5(uuid()),0,$maxLenth);
    $dirname = strtolower(setFilePathFormat(mb_substr($dirname, 0, $maxLenth-5).'_'.mb_substr(uuid(),3,4)));
...
function setFilePathFormat($str, $tolower = false){
    $str = translitRichStr($str);
...
function & getCategoriesTree($lang, $pid=0, $level=0, $showOnlyActive=true, $module=false, $order='', $extraterm='', $arPath=array(), $itemsTable = '', $columnName = '') {
    $module    = trim((string)$module);
    $extraterm = trim((string)$extraterm);
    $items     = array();
    $query     = "SELECT t.*, ljt1.`title_{$lang}` as pagetitle, ljt2.`title_{$lang}` as menutitle
                  ".($itemsTable && $columnName ? ', (SELECT COUNT(id) FROM '.$itemsTable.' WHERE '.$columnName.'=t.id) itemsCnt ' : '')."
                  FROM `".MAIN_TABLE."` t
    ...
    if(mysql_num_rows($result) > 0){
        while($row = mysql_fetch_assoc($result)){
            ...
            $row['level']      = $level;
            $row['items']      = array();
            $row['margin']     = $level==0 ? '&nbsp;' : '&nbsp;'.str_repeat('&middot;&nbsp;&middot;&nbsp;&middot;&nbsp;&middot;&nbsp;&middot;&nbsp;', $level);
            $row['childrens']  = getCategoriesTree($lang, $row['id'], $level+1, $showOnlyActive, $module, $order, '', $row['arPath'], $itemsTable, $columnName);
            $items[]           = $row;
        }
    } return $items;
}
...
function getRowItemsInKeyArray($keyname, $table, $selcolnames = '*', $whereoptions = '', $orderConditions = '', $limitConditions = '') {
    if(empty($keyname)) $keyname = 'id';
    $items = array();
    $query = "SELECT $selcolnames FROM $table".((!empty($whereoptions)) ? " $whereoptions" : '').((!empty($orderConditions)) ? " $orderConditions" : '').((!empty($limitConditions)) ? " $limitConditions" : '');
    $result = mysql_query($query);
    while ($row = mysql_fetch_assoc($result)) { 
        if(!isset($items[$row[$keyname]])) $items[$row[$keyname]] = array();
...
function getRowItemsInKeyArrayValue($keyname, $valuename, $table, $selcolnames = '', $whereoptions = '', $orderConditions = '', $limitConditions = '') {
    if(empty($selcolnames)) $selcolnames = "$keyname, $valuename";
    $items = array();
    $query = "SELECT $selcolnames FROM $table".((!empty($whereoptions)) ? " $whereoptions" : '').((!empty($orderConditions)) ? " $orderConditions" : '').((!empty($limitConditions)) ? " $limitConditions" : '');
    $result = mysql_query($query);
...
function getModule($module) {
    $items = getModules();
    return array_key_exists($module, $items) ? $items[$module] : null;
}
...
function getSettings() {  
    ...
    if(!empty($items)){
        $item  = (object)$items;
        $item->sitePhone      = trim($item->sitePhone);
        $item->siteEmail      = trim($item->siteEmail);
        $item->adwordsEmail   = trim($item->adwordsEmail);
        $item->notifyEmail    = trim($item->notifyEmail);
        $item->copyright      = preg_replace("/(\d{4}\s?)[-|–](\s?\d{4})/", '\1-'.date('Y'), $item->copyright);
        $item->eurRate        = floatval($item->eurRate);
        $item->pricePrecision = intval($item->pricePrecision);
    } else {
        $item = new stdClass();
    } return $item;
}
...
function sendMultipartMail($mail_to, $mail_from, $subject, $html, $path, $contentType = 'plain') {
    ...
    if (!empty($arFiles)) {
        foreach ($arFiles as $arFile) {
            ...
            $multipart .= "Content-Type: application/octet-stream; name=\"{$arFile["name"]}\"$EOL";
            $multipart .= "Content-Transfer-Encoding: base64$EOL";
            $multipart .= "Content-Disposition: attachment; filename=\"{$arFile["name"]}\"$EOL";
            $multipart .= $EOL; // раздел между заголовками и телом прикрепленного файла !! IMPORTANT !!
            $multipart .= chunk_split(base64_encode($arFile["file"]));
        }
    }
    # Finished
    $multipart .= "$EOL--$boundary--$EOL"; // finish with two eol's for better security. see Injection.
    # SEND THE EMAIL
    if(mail($mail_to, $subject, $multipart, $headers)) { return true; }//если письмо отправлено
...
//  Проверяет дату
    if (preg_match("/[0-3][0-9][-][0-1][0-9][-][2][0][0-5][0-9]/u", $date)) return true;
    else return false;
}
...
//  Проверяет строку на наличие значений в $arr
    ...
    if (preg_match("/[".implode("|", $arr)."]/u", $str)) return true;
    else return false;
}
...
function translitStr($str) {
    $str = mb_strtr($str, "абвгдеёзийклмнопрстуфхыэ_", "abvgdeeziyklmnoprstufhiei");
    $str = mb_strtr($str, "АБВГДЕЁЗИЙКЛМНОПРСТУФХЫЭ_", "ABVGDEEZIYKLMNOPRSTUFHIEI");
    $arChars = array(
        "ж"=>"zh", "ц"=>"ts", "ч"=>"ch", "ш"=>"sh",
        "щ"=>"sch","ь"=>"", "ъ" => "", "ю"=>"yu", "я"=>"ya",
...
function rel2abs($curdir, $relpath) {
    ...
    if (($p = strpos($res, "\0")) !== false)
        $res = mb_substr($res, 0, $p);

    while (strpos($res, "/./") !== false)
        $res = str_replace("/./", "/", $res);

    while (($pos = strpos($res, "../")) !== false) {
        $lp = mb_substr($res, 0, $pos - 1);
        $posl = bxstrrpos($lp, "/");
        if ($posl === false) return;
        $lp = mb_substr($lp, 0, $posl + 1);
        $rp = mb_substr($res, $pos + 3);
        $res = $lp . $rp;
    }
