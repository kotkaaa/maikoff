...
function getMenu($type, $pid=false, $incLevels=0, $level=1, $arPath=array()){
    global $catid, $UrlWL;
    $menu = array();
    $query = 'SELECT `id`, `pid`, `redirectid`, `title`, `image`, `image_menu`, `image_icon`, `menutype`, `separator`, `show_on_top`, `pagetype`, `module`, TRIM(`redirecturl`) `redirecturl`, TRIM(`seo_path`) `seo_path`
                FROM `'.MAIN_TABLE.'`
                WHERE `active` = 1 AND `menutype`='.$type.($pid!==false ? ' AND `pid`='.$pid : '').' 
                ORDER BY `order`, `id`';
    $result = mysql_query($query);
    while ($row = mysql_fetch_assoc($result)) {
        $row = array_merge($row, UrlWL::getCategoryByIdWithSeoPath($row["id"]));
        $row['redirecturl']   = trim($row['redirecturl']);
        $row['redirectid']    = intval($row['redirectid']);
        if ($row['redirecturl']!='') $row['redirectid'] = 0;
        $item = $row['redirectid'] ? UrlWL::getCategoryByIdWithSeoPath($row['redirectid']) : array();
        $row['arPath']    = empty($row['arPath']) ? $arPath : $row['arPath'];
//        $row['arPath'][]  = trim($row['seo_path']);
        ...
        $row['subcategories'] = (!$incLevels || $level<$incLevels) ? getMenu($type, $id, $incLevels, $level+1, array()) : array();
        $row['opened']        = ($id==$catid || IsChild($row['id'], $catid)) ? 1 : 0;
        $row['level']         = $level;
        if (!empty($row['image']) and file_exists(MAIN_CATEGORIES_DIR.$row['image'])) { 
            $row['image'] = MAIN_CATEGORIES_URL_DIR.$row['image']; 
        }
        if (!empty($row['image_menu']) and file_exists(MAIN_CATEGORIES_DIR.$row['image_menu'])) { 
            $row['image_menu'] = MAIN_CATEGORIES_URL_DIR.$row['image_menu']; 
        }
        if (!empty($row['image_icon']) and file_exists(MAIN_CATEGORIES_DIR.$row['image_icon'])) { 
            $row['image_icon'] = MAIN_CATEGORIES_URL_DIR.$row['image_icon']; 
        } $menu[] = $row;
    } return $menu;
}
