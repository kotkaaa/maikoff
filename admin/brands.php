        ...
        if ($Validator->foundErrors()) {
            ...
            if($result){
                ...
                if (!empty($_POST['series'])) {
                    $order = 1;
                    foreach ($_POST['series'] as $key => $arValue){
                        $valueItem = !empty($arValue['id']) ? getItemRow(SERIES_TABLE, '*', 'WHERE `id`='.$arValue['id']) : array();
                        // SEO path manipulation
                        if (($arValue["seo_path"] = trim($arValue["seo_path"]))!=='' and ($arValue["seo_path"] = Url::stringToUrl($arValue["seo_path"]))!=='') {
                            $arValue['seo_path']  = $UrlWL->strToUniqueUrl($DB, $arValue['seo_path'], $arPostData["seo_path"], SERIES_TABLE, (empty($valueItem['id']) ? 0 : $valueItem['id']), empty($valueItem['id']));
                        }
                        $arData = array(
                            'brand_id'  => $itemID,
                            'title'     => $arValue['title'],
                        ...
                        $result = $DB->postToDB($arData, SERIES_TABLE, !empty($valueItem) ? 'WHERE `id`='.$valueItem['id'] : '', array(), (!empty($valueItem) ? 'update' : 'insert'), (!empty($valueItem) ? false : true));
                        $arResults[] = !empty($valueItem) ? $valueItem['id'] : $result;
                    }
                }
                deleteDBLangsSync(SERIES_TABLE, 'WHERE `brand_id`='.$itemID.(!empty($arResults) ? ' AND `id` NOT IN('.  implode(',', $arResults).')' : ''));
...
if ($task=='addItem' || $task=='editItem') {
    if (!$itemID) {
        ...
        if (!$result) {
            ...
            $item['series']      = getRowItemsInKey("id", SERIES_TABLE, '*', 'WHERE `brand_id`='.$itemID, 'ORDER BY `order`');
            if (!empty($item['series'])) {
                foreach ($item['series'] as $key => $val) {
                    $item['series'][$key]['edit'] = true;
                }
            }
            $item['imagesCount'] = getValueFromDB(BRANDS_GALLERY_TABLE, "COUNT(*)", "WHERE `pid`=$itemID", "cnt");
        }
    }
    if (!empty($_POST)) $item = array_merge($item, $_POST);
    $item['seriesMaxID'] = intval(getValueFromDB(SERIES_TABLE, 'MAX(id)+100', '', 'max'));
