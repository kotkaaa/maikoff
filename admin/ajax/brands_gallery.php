...
require_once('includes/classes/ImagesUpload.php');
# ##############################################################################
    ...
    if($hasAccess) {
        if(!$arItem['diffTables']) {
            ...
            if(!$result) {
                ...
                if(empty($default)) updateRecords($arItem['ftable'], '`isdefault`=1', 'WHERE `pid`='.$pid. ' ORDER BY `itemorder` LIMIT 1');
...
elseif($task=='editItems' and isset($_POST['submit_order']) and $arItem['diffTables']) {
    if($hasAccess) {
        ...
        foreach($_POST['arItemsId'] as $itemID) {
            ...
            $arPostData = array(
                'itemorder' => $_POST['arOrder'][$itemID],
        ...
        if($new_name and file_exists($arrPageData['files_path'].$new_name)) {
            if($arItem['diffTables']) {
                $arPostData = array(
                    ...
                    'pid'       => $pid,
                    'itemorder' => intval(getValueFromDB($arItem['ftable']." t", 'COUNT(*)', 'WHERE `pid`="'.$pid.'"', 'count')),
                    'title'     => ''
                );
...
$arrPageData['headScripts'][]   = '<script src="/js/admin/swfobject.js" type="text/javascript"></script>';
...
if($arItem['diffTables']){
    ...
    $order = $arItem['diffTables'] ? "ORDER BY t.`itemorder`" : '';
