...
// User verification
if($itemID && $action=='edit' && $objUserInfo->logined && $itemID!=$objUserInfo->id && !$UserAccess->isAdmin($objUserInfo->type)){ 
    $itemID = $objUserInfo->id;
}
// Edit and View User Data Limit For Registered User
if((!$itemID || $itemID!=$objUserInfo->id) && !$UserAccess->isUser($objUserInfo->type)){ 
...
if($itemID && $action=='edit' && !empty($_POST)) {
    ...
    if ($UserAccess->isAdmin($objUserInfo->type) === false)
...
// List Items
    ...
    $where = '  WHERE t.`active`=1'.((!$objUserInfo->logined || !$UserAccess->isAdmin($objUserInfo->type)) ? ' AND t.`isnew`=0 AND t.`type`="'.USER_TYPE_USER.'" ' : '');
