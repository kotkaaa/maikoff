...
if (!$is_safe_mode and function_exists('set_time_limit')) set_time_limit(TIME_LIMIT);
...
} elseif (!empty($_POST['login']) and isset($_POST['pass'])) {
...
if (!$auth or (isset($_SERVER['QUERY_STRING']) and $_SERVER['QUERY_STRING'] == 'reload')) {
...
if (!file_exists(PATH) and !$is_safe_mode) {
    mkdir(PATH, 0777) or trigger_error("Не удалось создать каталог для бекапа", E_USER_ERROR);
}
...
class dumper {
    ...
    function dumper() {
        ...
        preg_match("/^(\d+)\.(\d+)\.(\d+)/u", mysql_get_server_info(), $m);
        ...
        if (preg_match("/^(forced->)?(([a-z0-9]+)(\_\w+)?)$/u", RESTORE_CHARSET, $matches)) {
    ...
    function backup() {
        ...
        $this->SET['tables_exclude']  = !empty($_POST['tables']) and $_POST['tables']{0} == '^' ? 1 : 0;
        ...
        while($row = mysql_fetch_array($result)) {
            ...
            if (!empty($tbls)) {
                foreach($tbls AS $table) {
                    $exclude = preg_match("/^\^/u", $table) ? true : false;
                    if (!$exclude) {
                    ...
                    if ($exclude and preg_match("/{$table}$/i", $row[0])) {
                        $status = -1;
                    }
                }
            } else {
        ...
        if ($this->mysql_version > 40101 and CHARSET != 'auto') {
        ...
        // Выставляем кодировку соединения соответствующую кодировке таблицы
            if ($this->mysql_version > 40101 and $tab_charset[$table] != $last_charset) {
                if (CHARSET == 'auto') {
            ...
            while($col = mysql_fetch_row($result)) {
                $NumericColumn[$field++] = preg_match("/^(\w*int|year)/u", $col[1]) ? 1 : 0;
            }
            $fields = $field;
            $from = 0;
            $limit = $tabsize[$table];
            ...
            if ($tabinfo[$table] > 0) {
                ...
                while(($result = mysql_query("SELECT * FROM `{$table}` LIMIT {$from}, {$limit}")) and ($total = mysql_num_rows($result))) {
    ...
    function restore() {
        ...
        if(preg_match("/^(.+?)\.sql(\.(bz2|gz))?$/u", $file, $matches)) {
            if (isset($matches[3]) and $matches[3] == 'bz2') {
                $this->SET['comp_method'] = 2;
            } elseif (isset($matches[2]) and$matches[3] == 'gz') {
        ...
        if ($this->mysql_version > 40101 and (CHARSET != 'auto' or $this->forced_charset)) { // Кодировка по умолчанию, если в дампе не указана кодировка
        ...
        while(($str = $this->fn_read_str($fp)) !== false) {
            if (empty($str) or preg_match("/^(#|--)/u", $str)) {
                if (!$is_skd and preg_match("/^#SKD101\|/u", $str)) {
                    $info = explode("|", $str);
            ...
            if (!$insert and preg_match("/^(INSERT INTO `?([^` ]+)`? .*?VALUES)(.*)$/i", $str, $m)) {
                ...
                if ($insert) {
                    $i++;
                    $t++;
                    if ($is_skd and $info[$index] > $limit and $t % $limit == 0) {
                        echo tpl_s($i / $info[$index], $t / $info[4]);
                    }
                }
            }

            if (!$insert and preg_match("/^CREATE TABLE (IF NOT EXISTS )?`?([^` ]+)`?/i", $str, $m) and $table != $m[2]) {
                $table = $m[2];
                $insert = '';
                $tabs++;
                $is_create = true;
            ...
            if ($sql) {
                if (preg_match("/;$/u", $str)) {
                    ...
                    if (empty($insert)) {
                        ...
                        // Выставляем кодировку соединения
                            if (preg_match("/(CHARACTER SET|CHARSET)[=\s]+(\w+)/i", $sql, $charset)) {
                                if (!$this->forced_charset and $charset[2] != $last_charset) {
                                    if (CHARSET == 'auto') {
                    ...
                    } elseif($this->mysql_version > 40101 and empty($last_charset)) { // Устанавливаем кодировку на случай если отсутствует CREATE TABLE
                ...
                if ($query_len >= 65536 and preg_match("/,$/u", $str)) {
                    $sql = rtrim($insert . $sql, ",");
                    $execute = 1;
                }
                if ($execute) {
    ...
    function file_select() {
        ...
        if (is_dir(PATH) and $handle = opendir(PATH)) {
            while (false !== ($file = readdir($handle))) {
                if (preg_match("/^.+?\.sql(\.(gz|bz2))?$/u", $file)) {
                    $files[$file] = $file;
                }
            }
            closedir($handle);
    ...
    function fn_read_str($fp) {
        ...
        if ($pos < 1) {
            while (!$string and ($str = $this->fn_read($fp))) {
                $pos = strpos($str, "\n", 0);
                if ($pos === false) {
                    $this->file_cache .= $str;
                } else {
...
function SXD_errorHandler($errno, $errmsg, $filename, $linenum, $vars) {
    ...
    if (preg_match("/chmod\(\).*?: Operation not permitted/u", $errmsg)) return true;
    $dt = date("Y.m.d H:i:s");
