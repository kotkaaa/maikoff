...
<form method="post" action="<{$arrPageData.current_url|cat:$arrPageData.filter_url|cat:"&task=editItems"}>" name="editItems" id="ajaxEditForm"  enctype="multipart/form-data">
    ...
    <table width="100%" cellspacing="0" cellpadding="0" border="0" class="sheet">
        <tbody>
            <tr>
                <td valign="top" width="50%">
                    <table width="100%" cellspacing="1" cellpadding="0" border="0" class="sheet">
                        <tbody>
                            <tr>
                                <td>
                                    <input id="file" type="file" name="file"><br/>
<{if isset($arrPageData.image)}>
                                    <strong>Выбрано:</strong> <{$arrPageData.image.name}>&nbsp;&nbsp; <label><input type="checkbox" value="1" name="allLangs"/> загрузить для всех языков</label><br/>
<{/if}>
...
<{if $items[i].allLangs}>
                                    | <a href="<{$arrPageData.current_url|cat:$arrPageData.filter_url|cat:"&task=deleteItem&langs=all&itemID="|cat:$items[i].id}>" onclick="return confirm('Вы уверены что хотите удалить файл?');" title="Delete!">Удалить со всех языков</a>
<{/if}>
<{/if}>
<{/section}>
<{/if}>
...
<{if !empty($items) and $arItem.diffTables}>
                <td valign="top" width="50%">                   
                    <table width="100%" border="0" cellspacing="1" cellpadding="0" class="list colored" id="operationTbl">
                        <tr>
                            ...
                            <td id="headb" align="center" width="22" <{if $arrPageData.pmodule=="brands"}>style="display: none;"<{/if}>>Глав</td>
...
<{if $arrPageData.pmodule=="brands"}>
                            <td id="headb" align="left">ссылка</td>
<{/if}>
...
<{section name=i loop=$items}>
                         <tr>
                            ...
                            <td align="center" <{if $arrPageData.pmodule=="brands"}>style="display: none;"<{/if}>>
...
<{/if}>
                            ...
                            <td>
                                <input type="text" size="<{if $arrPageData.pmodule=="brands"}>20<{else}>45<{/if}>" class="field" name="arTitle[<{$items[i].id}>]" value="<{$items[i].title}>" />
                            </td>
<{if $arrPageData.pmodule=="brands"}>
                            <td>
                                <input type="text" size="20" class="field" name="arHref[<{$items[i].id}>]" value="<{$items[i].href}>" />
                            </td>
<{/if}>
                            <td align="center">
