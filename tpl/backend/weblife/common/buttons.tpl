...
<{if $task=='editItem' && $item.id > $deleteIDLimit && !isset($disableDelete) && (!isset($item.module) || (array_key_exists($item.module, $arrModules) && $item.id != $arrModules[$item.module].id)) }>
