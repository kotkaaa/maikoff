...
<div class="breadcrumb">
    <{if $arrPageData.task=='addItem' || $arrPageData.task=='editItem'}>
        ...
        <{if $arrPageData.module=='catalog' && isset($arrPageData.arModel)}>
            <{$arrPageData.path_arrow}> 
            <a href="/admin.php?module=models" title="">Модели</a>
        <{/if}>
        <{$arrPageData.path_arrow}> 
        <a href="<{$arrPageData.admin_url}>" title=""><{$title}></a>
        <{$arrPageData.path_arrow}> 
        <{if $arrPageData.task=='addItem'}>Создание<{else}>Редактирование №<{$item.id}><{/if}>
