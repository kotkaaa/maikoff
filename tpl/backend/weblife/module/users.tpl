<{* +++++++++++++++++ START HEAD ++++++++++++++++++++++ *}>
<{include file='common/module_head.tpl' title='Администраторы' creat_title=$smarty.const.ADMIN_CREATING_NEW_USER edit_title=$smarty.const.ADMIN_EDIT_USER}>
<{* +++++++++++++++++ END HEAD ++++++++++++++++++++++ *}>
                    ...
                    <table width="100%" border="0" cellspacing="1" cellpadding="0" class="" align="center">
                        <tr>
                            <td style="padding-left:30px;padding-top:20px;" valign="top">
                                ...
                                <div class="inline bigger">Доступ: <span class="required">*</span> </div>
                                <select name="type" class="field requirefield" style="width:132px; height:19px;"<{if $item.id==1 OR !$UserAccess->isAdmin($objUserInfo->type)}> disabled="disabled"<{/if}>>
                                    <{section name=i loop=$arrUserTypes}>
                                        <option value="<{$arrUserTypes[i].name}>"<{if $item.type==$arrUserTypes[i].name || ($arrPageData.task=='addItem' && $UserAccess->isUser($item.type))}>  selected<{/if}>> &nbsp; <{$arrUserTypes[i].title}> </option>
                                    <{/section}>
                                </select><br/><br/>

                                <div class="inline bigger"><{$smarty.const.USERS_ENABLED}>: <span class="required">*</span> </div>
                                <select name="active" class="field requirefield" style="width:132px; height:19px;"<{if $item.id==1 OR !$UserAccess->isAdmin($objUserInfo->type)}> disabled="disabled"<{/if}>>
                                    <option value="1"> <{$smarty.const.OPTION_YES}> </option>
                                    <option value="0"<{if $item.active==0}> selected<{/if}>> <{$smarty.const.OPTION_NO}> </option>
                                </select><br/><br/>

                                <div class="inline bigger"><{$smarty.const.USERS_LOGIN}>: <span class="required">*</span> </div>
                                <input class="field requirefield" size="90" name="login" type="text" value="<{$item.login}>"<{if $item.id==1 OR !$UserAccess->isAdmin($objUserInfo->type)}> readonly<{/if}> />
                                <input name="old_login" size="70" type="hidden" value="<{$item.login}>" /><br/><br/>

                                <{if $arrPageData.task=='addItem'}>
                                    <div class="inline bigger"><b><{$smarty.const.USERS_PASS}>: </b><span class="required">*</span> </div>
                                    <input class="field requirefield" size="90" name="pass"  type="password" value="" /><br/><br/>
                                    <div class="inline bigger"><b><{$smarty.const.USERS_CONFPASS}>: </b><span class="required">*</span> </div>
                                    <input class="field requirefield" size="90" name="confpass" type="password" value="" /><br/><br/>
                                <{elseif $arrPageData.task=='editItem'}>
                                    <{if $UserAccess->isAdmin($objUserInfo->type)}> 
                                        <div class="inline bigger">Старый пароль: <{if !$UserAccess->isAdmin($objUserInfo->type)}><span class="required">*</span><{/if}></div>
                                        <input class="field<{if !$UserAccess->isAdmin($objUserInfo->type)}> requirefield<{/if}>" name="old_pass"  size="90" type="password" value="" /><br/><br/>
                                    <{/if}>
                                    <div class="inline bigger"><{$smarty.const.USERS_NEW_PASS}>:</div>
                                    <input class="field" name="pass" size="90" type="password" value="" /><br/><br/>
                                    <div class="inline bigger"><{$smarty.const.USERS_CONFPASS}>:</div>
                                    <input class="field" name="confpass" size="90" type="password" value="" /><br/><br/>
                                <{/if}>
                                <div class="inline bigger left"><{$smarty.const.USERS_AUTO_PASS}></div>
                                ...
                                <div class="clear"></div>
                                <small style="margin-left:152px;"><{$smarty.const.USERS_COPY_PASS_BEFORE_SAVE}></small><br/><br/>

                                <div class="inline bigger"><{$smarty.const.USERS_SNAME}>: <span class="required">*</span></div>
                                <input class="field requirefield" name="surname" size="90" type="text" value="<{$item.surname}>" /><br/><br/>

                                <div class="inline bigger"><{$smarty.const.USERS_FNAME}>: <span class="required">*</span> </div>
                                <input class="field requirefield" name="firstname" size="90" type="text" value="<{$item.firstname}>" /><br/><br/>

                                <div class="inline bigger"><{$smarty.const.USERS_MNAME}>: </div>
                                <input class="field" name="middlename" type="text" size="90" value="<{$item.middlename}>" /><br/><br/>

                                <div class="inline bigger"><{$smarty.const.USERS_PHONE}>: </div>
                                <input class="field" name="phone" type="text" size="90" value="<{$item.phone}>" /><br/><br/>

                                <div class="inline bigger"><{$smarty.const.USERS_MAIL}>: <span class="required">*</span> </div>
                                ...
                                <div class="inline bigger"><{$smarty.const.LABEL_ADDRESS}>: </div>
                                <input class="field" name="address" type="text" size="90" value="<{$item.address}>" /><br/><br/>

                                <strong><{$smarty.const.USERS_DESCR}>: </strong><br/><br/>
                                <textarea name="descr" id="descr" style="width:632px; height:60px;" class="field"><{$item.descr}></textarea><br/><br/>
                            </td>
                            <td rowspan="0" class="buttons_row" valign="top" width="160" align="center">
                                <!-- ++++++++++ Start Buttons ++++++++++++++++++++++++++++++++++++++++++++++ -->
            ...
            $.getJSON(
                ...
                function (data) {
                    if (data.code) {
                        ...
                        alert("<{$smarty.const.USERS_GENERATE_PASS_ERROR}>");
                }
            );
        }
    //-->
...
<{else}>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0" class="list colored">
        <tr>
            ...
            <td id="headb" align="center"  width="100"><{$smarty.const.USERS_LOGIN}></td>
            <td id="headb" align="left"><{$smarty.const.USERS_FULL_NAME}></td>
            <td id="headb" align="center" width="150"><{$smarty.const.USERS_MAIL}></td>
            <td id="headb" align="center" width="150"><{$smarty.const.USERS_PHONE}></td>
            <td id="headb" align="center" width="150"><{$smarty.const.USERS_ACCESS}></td>
        ...
        <{section name=i loop=$items}>
            <tr>
                ...
                <td align='center'><{$items[i].login}></td>
                <td align="left">
                    <a href="<{$arrPageData.current_url|cat:"&task=editItem&itemID="|cat:$items[i].id}>" title="Просмотр">
                ...
                <td align='center'>
                    <{if $items[i].id==1 && !$UserAccess->isAdmin($objUserInfo->type)}>
                ...
                <td align='center'>
                    <{if $items[i].id==1 OR !$UserAccess->isAdmin($objUserInfo->type)}>
                        <{$smarty.const.HEAD_DENIED}>
                    <{else}>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0" class="list colored">
        ...
        <{section name=i loop=$arrNewItems}>
            <tr>
                ...
                <td align='center'>
                    <a href="<{$arrPageData.current_url|cat:"&task=editItem&itemID="|cat:$arrNewItems[i].id}>" title="<{$smarty.const.USERS_ACTIVATION_VIEW}>">
