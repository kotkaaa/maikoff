        ...
        <ul class="tabs">
            <li <{if $arrPageData.tab=='main'}>class="active"<{/if}> id="tab_main">
                <table border="1" cellspacing="0" cellpadding="1" class="sheet">       
                    ...
                    <tr>
                        <td id="headb" align="left" width="175">Операции с кешем</td>
                        <td class="padding">
                           <a href="javascript:void(0)" onclick="if(window.confirm('<{$smarty.const.LABEL_QUESTION_TO_DO}> что хотите очистить ВЕСЬ кеш?')) {flushCache(this);} return false;">
                               Очистить весь кеш
                           </a>
                        </td>
                        <td class="buttons_row" valign="top" width="144"></td>
            ...
            <li id="tab_modules" <{if $arrPageData.tab=='modules'}>class="active"<{/if}>>
                <table border="1" cellspacing="0" cellpadding="1" class="list colored">
                    <tr>
                        <td>
                             <table border="1" cellspacing="0" cellpadding="1" class="list colored"> 
                                 <tr>
...
<script type="text/javascript">   
    ...
    function flushCache(obj, options) {
        $('#messages').attr('class', 'hidden_block').html('');
        $.ajax({
            url: '/interactive/ajax.php?zone=admin&action=ajaxCacheFlush',
            type: 'GET',
            dataType: 'json',
            data: options||{},
            success: function(json) {
                $('#messages').attr('class', 'info').html('Кеш очищен');
            }
        }).error(function(jqXHR, textStatus, errorThrown ){
            var errorMessage = 'Ошибка при очистке кеша! ';
            if(textStatus) errorMessage += 'Результат запроса: ' + textStatus;
            $('#messages').attr('class', 'error').html(errorMessage);
        });
    };
