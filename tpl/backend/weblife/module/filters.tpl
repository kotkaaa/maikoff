            ...
            <ul class="tabs">
                <li class="active" id="tab_main">
                    <table border="1" cellspacing="0" cellpadding="1" class="sheet">
                        ...
                        <tr>
                            <td id="headb" align="left" width="175">Алиас</td>
                            <td>
                                <select name="alias">
                                    <option value="">Выберите алиас, если нужно</option>
                                    <{foreach from=BreadcrumbsTemplates::getAliases() item=title key=alias}>
                                        <option value="<{$alias}>"<{if $item.alias == $alias}> selected<{/if}>><{$title}></option>
                                    <{/foreach}>
                                </select>
                            </td>
...
<{/section}>
                                ...
                                <label style="display: none;">
                                    <input type="checkbox" id="rangeTrigger" class="nosize_field" name="is_range" value="1" <{if empty($item.arRanges) AND ($item.tid==UrlFilters::TYPE_TEXT OR UrlFilters::isIndependentFilter($item.tid))}>disabled<{/if}> <{if ($item.tid==UrlFilters::TYPE_PRICE OR $item.tid==UrlFilters::TYPE_NUMBER) AND !empty($item.arRanges)}>checked<{/if}>/> Диапазон
                                </label>
                            </td>
                            <td class="buttons_row"></td>
                        </tr>
                        ...
                        <tr>
                            <td colspan="2">
                                <table id="attrTable" border="1" cellspacing="0" cellpadding="1" class="sheet">
                                    <tr>
...
<{/section}>
                                ...
                                <table id="rangeTable" border="1" cellspacing="0" cellpadding="1" class="sheet">
                                    <tbody>
...
<{if empty($item.arRanges) OR $item.tid==UrlFilters::TYPE_TEXT OR UrlFilters::isIndependentFilter($item.tid)}>
            $(RangeTable).hide();
<{/if}>
<{if $item.aid==0}>        
            $(AttrTable).hide();
<{/if}>
        ...
        function addRangeRow() {
            var range_row = $('table#rangeTable > tbody > tr').length + 1,
                target = $('table#rangeTable > tbody'),
                html;
    ...
    <form method="POST" action="<{$arrPageData.current_url|cat:"&task=reorderItems"}>" name="reorderItems">
        <table width="100%" border="0" cellspacing="1" cellpadding="0" class="list colored">
            <tr>
                ...
                <td id="headb" align="center" width="120">Тип</td>
